...
int adcTransfer(XSpi *InstancePtr, XGpio *GpioInstancePtr, u8 *wBuffer, u8 *rBuffer,  u32 ByteToTransfer,  int SlaveSelect)
{
	int Status;
	...
	Status = XSpi_Transfer(InstancePtr, (u8 *)wBuffer, (u8 *)rBuffer, ByteToTransfer);
	if (Status != XST_SUCCESS)
	{
	...
	xil_printf("POMIARY:\n\r");
	xil_printf("1)\n\r");
	xil_printf("WriteData: %x \n\rReadData: %d \n\r",(wBuffer[0] << 8) | wBuffer[1], rBuffer[0]  | (rBuffer[1] << 8));
	xil_printf("2)\n\r");
	xil_printf("WriteData: %x \n\rReadData: %d \n\r",(wBuffer[2] << 8) | wBuffer[3], rBuffer[2]  | (rBuffer[3] << 8) );
	xil_printf("3)\n\r");
	xil_printf("WriteData: %x \n\rReadData: %d \n\r",(wBuffer[4] << 8) | wBuffer[5], rBuffer[4]  | (rBuffer[5]  << 8));
	xil_printf("4)\n\r");
	xil_printf("WriteData: %x \n\rReadData: %d \n\r",(wBuffer[6] << 8) | wBuffer[7], rBuffer[6]  | (rBuffer[7] << 8));
