...
int adcTransfer(u16 *rBuffer, int SlaveSelect)
{
	int Status;
	u8 *buffer = ReadBuffer;
	u8 wBuffer[8] = {0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00};

	Status = XSpi_SetSlaveSelect(&SpiInstance, SlaveSelect);
	if (Status != XST_SUCCESS)
	{
	...
	Status = XSpi_Transfer(&SpiInstance, (u8 *)wBuffer, (u8 *)buffer, 2);
	if (Status != XST_SUCCESS)
	{
	...
	TransferProgress = TRUE;
	Status = XSpi_Transfer(&SpiInstance, wBuffer+2, buffer+2, 2);
	if (Status != XST_SUCCESS)
	{
		xil_printf("SPI Transfer FAILURE %x \n\r", Status);
	}
	...
	TransferProgress = TRUE;
	Status = XSpi_Transfer(&SpiInstance, wBuffer+4, buffer+4, 2);
	if (Status != XST_SUCCESS)
	{
		xil_printf("SPI Transfer FAILURE %x \n\r", Status);
	}
	...
	*rBuffer = (((*(buffer)) | (*(buffer+1)<< 8)) & 0xfff);
	*(rBuffer+1) = (((*(buffer+2) ) | (*(buffer+3) << 8)) & 0xfff);
	*(rBuffer+2) = (((*(buffer+4) ) | (*(buffer+5)<< 8)) & 0xfff);
	*(rBuffer+3) = (((*(buffer+6) ) | (*(buffer+7)<< 8)) & 0xfff);
//	*(rBuffer+4) = ((((*buffer+8) << 8) | (*(buffer+9))) & 0xfff);

	xil_printf("1:%d \n\r2:%d \n\r3:%d\n\r, 4:%d\n\r",(((*(buffer)) | (*(buffer+1)<< 8)) & 0xfff),
			(((*(buffer+2) ) | (*(buffer+3) << 8)) & 0xfff),(((*(buffer+4) ) | (*(buffer+5)<< 8)) & 0xfff),
			(((*(buffer+6) ) | (*(buffer+7)<< 8)) & 0xfff));
	return XST_SUCCESS;
}
