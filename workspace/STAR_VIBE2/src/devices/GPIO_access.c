...
int init_gpio()
{
	...
	XGpio_SetDataDirection(&GpioInstance, CMV_CHANNEL_OUTPUT, 0x00);  /* 8 Outputs */
	XGpio_SetDataDirection(&GpioInstance, CMV_CHANNEL_INPUT, 0b111111);	  /* 4 Inputs */
	 ...
	 */
	XGpio_InterruptEnable(&GpioInstance, INTR_CMV_MASK);
	XGpio_InterruptGlobalEnable(&GpioInstance);

	XGpio_DiscreteWrite(&GpioInstance, 2, 0x0);
